...
%% Introduce the package needed for the hyperlink
...
\newcommand{\hmwkTitle}{Lab 2}
\newcommand{\hmwkSubTitle}{Tiled Matrix Multiplication}
\newcommand{\hmwkDueDate}{April 27, 2018}
\newcommand{\hmwkClass}{EE 147}
\newcommand{\hmwkAuthorName}{Yixin Yang}
\newcommand{\hmwkAuthorID}{862004384}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Make title
\title{\vspace{2in}\textmd{\textbf{\hmwkClass:\ \hmwkTitle\thanks{Template from\ \texttt{www.tedpavlic.com}}\ifthenelse{\equal{\hmwkSubTitle}{}}{}{\\\hmwkSubTitle}}}\\\normalsize\vspace{0.1in}\small{Due\ on\ \hmwkDueDate}\\\vspace{0.1in}\large{\textit{}}}
\date{}
\author{\textbf{\hmwkAuthorName}\ \\\vspace{0.5in}\ \hmwkAuthorID}
...
executing? Assume a GPU which has 30 streaming multiprocessors.\\
    \textbf{Solution}\\
The maximun number of threads that could simultenously run in a SM is 
$16^2=256$. 
Besides, the GPU have 30 SMs. So, the total number of threads that could run 
simultaneously in the device is \[256\times30=7680\]

\end{homeworkProblem}

%% Problem 2
\begin{homeworkProblem}
Use nvcc –ptxas-options=``-v'' to report the resource usage of your
implementation. Note that the compilation will fail but you will still get a report
of the relevant information. Experiment with the Nvidia visual profiler, which is
part of the CUDA toolkit, and use it to further understand the resource usage. In
particular, report your branch divergence behavior and whether your memory
accesses are coalesced.
\begin{homeworkSection}{(a)}
The resource usage\\
\textbf{Solution}\\
  Using nvcc –ptxas-options=``-v'' to report the resource usage of my
  implementation. The result is shown as \hyperref[result1.png]{Fig.\ref{result1.png}}. It shows that 26 registers, 2048 butes smem, 360 bytes cmem were used.

  %% \begin{figure}
	%%   \begin{center}
	%% 	  \includegraphics[width=0.32\linewidth]{images//result1.png}
  ...
  \scalefig{result1.png}{0.5}{The resource usage}

\end{homeworkSection}

\begin{homeworkSection}{(b)}
Branch divergence behavior\\
\textbf{Solution}\\
Using nvprof --metrics branch\_efficiency
 to report the branch divergence
 behavior of my implementation. The result is shown as \hyperref[result2.png]{Fig.\ref{result2.png}}.\\
It shows that the branch efficiency is 100\%, which means there is no branch divergence.
   %% \begin{figure}
	 %%  \begin{center}
	 %%    \includegraphics[width=0.32\linewidth]{images//result2.png}
	 %%    \caption{The branch divergence behavior}
	 %%    \label{Fig:2}
	 %%  \end{center}
	 %%  \vspace{-0.5em}
   %% \end{figure}

   \scalefig{result2.png}{0.5}{The branch divergence behavior}

\end{homeworkSection}
%% \clearpage

\begin{homeworkSection}{(c)}
Whether your memory accesses are coalesced\\
\textbf{Solution}\\
 %% \clearpage
Using nvprof --metrics gld\_efficiency, gst\_efficiency
 to report the global memory load efficiency
and global memory store efficiency of my implementation.
The result is shown as \hyperref[result3.png]{Fig.\ref{result3.png}}.\\
Because all accessed
locations fall into the same burst section, only one DRAM request
will be made and the access is fully coalesced.
  %% \begin{figure}
	%%   \begin{center}
	%% 	  \includegraphics[width=0.32\linewidth]{images//result3.png}
	%% 	  \caption{Global memory eiificiency}
	%% 	  \label{Fig:3}
	%%   \end{center}
	%%   \vspace{-0.5em}
  %% \end{figure}

  \scalefig{result3.png}{0.5}{Global memory eiificiency}
...
%% Problem 3
\begin{homeworkProblem}
How many times is each element of the input matrices loaded during the
execution of the kernel?\\
\textbf{Solution}\\
Consider two matrices M and N, where M is m-by-k and N is k-by-n.
An element of input matrix M, for instans $M_{a,b}$, is loaded by all the threads that calculates elements at the a-row of matrix P.
An element of input matrix N, for instans $N_{c,d}$, is loaded by all the threads that calculates elements at the d-column of matrix P.\\
Consequently,\\
For $m \times k$ input matrix M, each element is loaded n times.\\
For $k \times n$ input matrix N, each element is loaded m times.
